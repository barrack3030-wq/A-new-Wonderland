...
function doGet(e){
  try{return jsonResponse({ok:true,service:'Banggai Wonderland CMS',version:'7.1-resilient-json-api',status:'online',timestamp:new Date().toISOString()});}
...
function doPost(e){
  try{
    ...
    if(data.action==='translate')return jsonResponse({ok:true,articles:translateArticle(data)});
...
function uploadImage(data){
  ...
  const path='public/images/blog/'+fileName; githubCreateBinaryFile(path,blob.getBytes(),'CMS: Add blog image '+fileName);
...
function translateArticle(data){
  const title=String(data.title||'').trim(),description=String(data.description||'').trim(),content=String(data.content||'').trim();
  if(!title)throw new Error('Judul artikel wajib diisi.'); if(!content)throw new Error('Isi artikel wajib diisi.');
  ...
  const pubDate=normalizeDate(data.pubDate||getToday()),cfg=getOpenAIConfig();
  const articles={id:{title:title,description:description||createPreview(content),seoTitle:String(data.seoTitle||title),seoDescription:String(data.seoDescription||description||createPreview(content)),image:image.url||DEFAULT_IMAGE,imageAlt:String(data.imageAlt||image.alt||''),imageSource:String(image.source||''),author:String(data.author||'Banggai Wonderland'),pubDate:pubDate,tags:normalizeTags(data.tags),content:content,preview:createPreview(content)}};
  const langs=Object.keys(TARGET_LANGUAGES);
  for(let start=0;start<langs.length;start+=BATCH_SIZE){
    const batch=langs.slice(start,start+BATCH_SIZE),requests=batch.map(function(lang){return translatorRequest(cfg,articles.id,TARGET_LANGUAGES[lang]);}),responses=UrlFetchApp.fetchAll(requests);
    responses.forEach(function(response,index){
      const lang=batch[index],status=response.getResponseCode(),raw=response.getContentText(); let dataOut;
      if(isTransientStatus(status)){const retry=aiRequestResilient(cfg,buildTranslationPayload(cfg,articles.id,TARGET_LANGUAGES[lang]),'Translate ['+lang+']');dataOut=retry.data;}
      else{if(status<200||status>=300)throw new Error('OpenAI error ['+lang+'] ('+status+'): '+raw); try{dataOut=JSON.parse(raw);}catch(e){throw new Error('Respons AI tidak valid untuk '+lang+'.');}}
      const translated=parseTranslationResponse(dataOut,lang);
      articles[lang]={title:translated.title||articles.id.title,description:translated.description||articles.id.description,seoTitle:translated.seoTitle||translated.title||articles.id.title,seoDescription:translated.seoDescription||translated.description||articles.id.description,image:articles.id.image,imageAlt:translated.imageAlt||articles.id.imageAlt,imageSource:articles.id.imageSource,author:'Banggai Wonderland',pubDate:articles.id.pubDate,tags:translated.tags.length?translated.tags:articles.id.tags,content:translated.content,preview:createPreview(translated.content)};
    });
...
function buildTranslationPayload(cfg,source,targetLanguage){
  ...
  return {model:cfg.model,input:[{role:'user',content:[{type:'input_text',text:prompt}]}],text:{format:{type:'json_object'}},store:false};
}
...
function parseTranslationResponse(response,lang){
  const output=extractAIText(response); if(!output)throw new Error('AI tidak mengembalikan hasil terjemahan untuk '+lang+'.');
  let parsed; try{parsed=JSON.parse(cleanJsonOutput(output));}catch(e){throw new Error('Hasil terjemahan bukan JSON valid untuk '+lang+'.');}
  if(!parsed.content||String(parsed.content).trim().length<100)throw new Error('Isi terjemahan '+lang+' tidak lengkap.');
  return {title:String(parsed.title||''),description:String(parsed.description||''),seoTitle:String(parsed.seoTitle||''),seoDescription:String(parsed.seoDescription||''),imageAlt:String(parsed.imageAlt||''),tags:normalizeTags(parsed.tags),content:String(parsed.content).trim()};
}
function aiRequestResilient(cfg,payload,label){
  let lastRaw='',lastStatus=0;
  for(let attempt=1;attempt<=MAX_RETRIES;attempt++){
    const result=rawAiFetch(cfg,payload);
    if(result.status>=200&&result.status<300){let data;try{data=JSON.parse(result.raw);}catch(e){throw new Error(label+': Respons OpenAI tidak valid.');}return {data:data,status:result.status};}
    lastRaw=result.raw;lastStatus=result.status; if(!isTransientStatus(result.status)||attempt===MAX_RETRIES)break;
    const delay=Math.min(30000,Math.pow(2,attempt-1)*1200+Math.floor(Math.random()*800)); Utilities.sleep(delay);
  }
...
function rawAiFetch(cfg,payload){
  ...
  const r=UrlFetchApp.fetch(cfg.url,options); return {status:r.getResponseCode(),raw:r.getContentText()};
}
...
function findRelevantImage(topic){
  ...
  if(lower.indexOf('paisupok')!==-1)queries.push('Paisupok Lake Banggai'); if(lower.indexOf('poganda')!==-1)queries.push('Poganda Beach Banggai');
  if(lower.indexOf('pulo dua')!==-1)queries.push('Pulo Dua Banggai'); if(lower.indexOf('kamumu')!==-1)queries.push('Kamumu Waterfall Banggai');
  if(lower.indexOf('piala')!==-1)queries.push('Piala Waterfall Luwuk Banggai'); queries.push(text+' Banggai Indonesia');
  for(let i=0;i<queries.length;i++){const r=searchWikimediaImages(queries[i]);if(r)return r;} return {url:DEFAULT_IMAGE,alt:'Banggai tropical destination in Indonesia',source:''};
}
function searchWikimediaImages(query){
  ...
  pages.forEach(function(page){
    const info=page.imageinfo&&page.imageinfo[0]; if(!info||!info.url)return; const mime=String(info.mime||'').toLowerCase();
    if(mime==='image/svg+xml'||mime.indexOf('image/')!==0)return; const title=normalizeSearchText(page.title||''); let score=0;
    words.forEach(function(w){if(title.indexOf(w)!==-1)score+=5;}); if(title.indexOf('banggai')!==-1)score+=5;
...
function extractAIText(response){
  ...
  if(response&&response.choices&&response.choices[0]&&response.choices[0].message&&response.choices[0].message.content)return response.choices[0].message.content; return '';
}
...
function publishArticles(articles){
  const files=[],first=articles.id; if(!first||!first.title||!first.content)throw new Error('Artikel Bahasa Indonesia tidak ditemukan.');
  const translationKey=getToday()+'-'+createSlug(first.title).substring(0,70);
  Object.keys(LANGUAGES).forEach(function(lang){const article=articles[lang];if(!article||!article.title||!article.content)throw new Error('Artikel bahasa '+lang+' tidak lengkap.');const slug=createSlug(article.title);if(!slug)throw new Error('Slug kosong untuk '+lang+'.');const path='src/content/blog/'+lang+'/'+article.pubDate+'-'+slug+'.md';githubCreateFile(path,createMarkdown(article,translationKey),'CMS: Add translated manual blog article ['+lang+'] '+article.title);files.push({language:lang,path:path,title:article.title});});
  return {files:files};
}
...
function githubCreateFile(path,content,message){
  ...
  if(!token||!owner||!repo)throw new Error('GitHub Script Properties belum lengkap.'); const url='https://api.github.com/repos/'+owner+'/'+repo+'/contents/'+path;
  const payload={message:message,content:Utilities.base64Encode(Utilities.newBlob(content).getBytes()),branch:branch}; const r=UrlFetchApp.fetch(url,{method:'put',contentType:'application/json',headers:{Authorization:'Bearer '+token,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28'},payload:JSON.stringify(payload),muteHttpExceptions:true});
  if(r.getResponseCode()<200||r.getResponseCode()>=300)throw new Error('GitHub error ('+r.getResponseCode()+'): '+r.getContentText()); return JSON.parse(r.getContentText());
}
function githubCreateBinaryFile(path,bytes,message){
  const p=PropertiesService.getScriptProperties(),token=p.getProperty('GITHUB_TOKEN'),owner=p.getProperty('GITHUB_OWNER'),repo=p.getProperty('GITHUB_REPO'),branch=p.getProperty('GITHUB_BRANCH')||'main';
  if(!token||!owner||!repo)throw new Error('GitHub Script Properties belum lengkap.'); const url='https://api.github.com/repos/'+owner+'/'+repo+'/contents/'+path;
  const payload={message:message,content:Utilities.base64Encode(bytes),branch:branch}; const r=UrlFetchApp.fetch(url,{method:'put',contentType:'application/json',headers:{Authorization:'Bearer '+token,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28'},payload:JSON.stringify(payload),muteHttpExceptions:true});
  if(r.getResponseCode()<200||r.getResponseCode()>=300)throw new Error('GitHub image error ('+r.getResponseCode()+'): '+r.getContentText()); return JSON.parse(r.getContentText());
}
