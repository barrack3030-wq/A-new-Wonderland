...
function doGet(){ return jsonResponse({ok:true,service:'Banggai Wonderland CMS'}); }
...
function uploadImage(data){
  ...
  return {url:'/images/blog/'+fileName,source:source||'',name:fileName};
}
...
function generateArticles(topic,selectedImage){
  ...
  const requests=langs.map(function(lang){return {url:'https://api.openai.com/v1/responses',method:'post',contentType:'application/json',headers:{Authorization:'Bearer '+apiKey},payload:JSON.stringify({model:OPENAI_MODEL,input:buildPrompt(topic,LANGUAGES[lang],image)}),muteHttpExceptions:true};});
...
function buildPrompt(topic,language,image){return `You are the professional travel content writer for Banggai Wonderland.\n\nWebsite: Banggai Wonderland\nSlogan: Discover hidden paradise of Banggai\n\nTOPIC:\n${topic}\n\nLANGUAGE:\nWrite the complete article in ${language}.\n\nFEATURED IMAGE:\n${image.url||DEFAULT_IMAGE}\n\nCreate a high-quality, natural, SEO-friendly travel article. Do not mention AI. Do not invent exact prices, schedules, distances or facilities when uncertain. Use a premium but natural travel-agency tone. Focus on Banggai, Luwuk and Banggai Islands when relevant. Use useful H2/H3 headings. The article body must be Markdown and contain no HTML. Keep proper destination names unchanged.\n\nReturn ONLY valid JSON:\n{"title":"SEO-friendly article title","description":"Short article description","seoTitle":"SEO title","seoDescription":"SEO meta description","image":"${image.url||DEFAULT_IMAGE}","imageAlt":"Descriptive image alt text","author":"Banggai Wonderland","pubDate":"${getToday()}","tags":["Banggai","Indonesia","Travel"],"content":"Complete Markdown article"}\nDo not wrap JSON in markdown fences.`;}
...
function searchWikimediaImages(query){
  ...
  let response; try{response=UrlFetchApp.fetch(WIKIMEDIA_API+'?'+params,{method:'get',muteHttpExceptions:true,headers:{'User-Agent':'BanggaiWonderlandCMS/1.0'}});}catch(e){return null;}
...
function extractOpenAIText(response){
  ...
  if(response.output&&Array.isArray(response.output))for(let i=0;i<response.output.length;i++){const item=response.output[i];if(!item.content)continue;for(let j=0;j<item.content.length;j++){const c=item.content[j];if(c.type==='output_text'&&c.text)return c.text;}}
  return '';
}
function validateArticle(article){['title','description','author','pubDate','content'].forEach(function(field){if(article[field]===undefined||article[field]===null||String(article[field]).trim()==='')throw new Error('Field artikel tidak lengkap: '+field);});}
function createPreview(content){return String(content||'').replace(/^#{1,6}\s+/gm,'').replace(/[*_`>]/g,'').replace(/\[([^\]]+)\]\([^\)]+\)/g,'$1').replace(/\s+/g,' ').trim().substring(0,320);}

function publishArticles(articles){
  const files=[],first=articles.id||articles.en||articles.es||articles.fr||articles.zh,translationKey=getToday()+'-'+createSlug(first.title).substring(0,70);
  Object.keys(LANGUAGES).forEach(function(lang){const article=articles[lang];if(!article||!article.title||!article.content)throw new Error('Artikel bahasa '+lang+' tidak lengkap.');const slug=createSlug(article.title);if(!slug)throw new Error('Slug kosong untuk '+lang);const path='src/content/blog/'+lang+'/'+getToday()+'-'+slug+'.md';githubCreateFile(path,createMarkdown(article,translationKey),'CMS: Add blog article ['+lang+'] '+article.title);files.push({language:lang,path:path,title:article.title});});
  return {files:files};
}
